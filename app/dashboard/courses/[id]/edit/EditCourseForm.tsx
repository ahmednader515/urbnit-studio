...
export function EditCourseForm({ courseId, initialData }: { courseId: string; initialData: InitialData }) {
  ...
  const [form, setForm] = useState({
    ...
    categoryNameAr: "",
    categoryNameEn: "",
    softwareToolsJson: initialData.softwareToolsJson,
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      softwareTools: form.softwareToolsJson.trim() || "[]",
    ...
    <form onSubmit={handleSubmit} className="mt-6 max-w-3xl space-y-8">
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-foreground)]">Software tools (JSON)</label>
            <textarea
              value={form.softwareToolsJson}
              onChange={(e) => setForm((f) => ({ ...f, softwareToolsJson: e.target.value }))}
              rows={4}
              placeholder='[{"id":"ps","label":"Ps","iconUrl":"https://..."}]'
              className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 font-mono text-xs"
            />
          </div>
          <div>
